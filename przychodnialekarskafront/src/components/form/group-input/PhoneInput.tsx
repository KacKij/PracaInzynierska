...
interface PhoneInputProps {
  ...
  placeholder?: string;
  onChange?: (phoneNumber: string, extention: string) => void;
...
const PhoneInput: React.FC<PhoneInputProps> = ({
  ...
  const handleCountryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const ext = countryCodes[newCountry]; // e.g. "+48"
    const updatedNumber = `${ext} ${phoneNumber.replace(/^\+\d+\s*/, "")}`; // normalize
  
    setPhoneNumber(updatedNumber);
    if (onChange) {
      onChange(updatedNumber, ext);
    }
  };
  
  const handlePhoneNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newPhonePart = e.target.value;
    const ext = countryCodes[selectedCountry];
    const fullNumber = `${ext} ${newPhonePart.replace(/^\+\d+\s*/, "")}`; // normalize
